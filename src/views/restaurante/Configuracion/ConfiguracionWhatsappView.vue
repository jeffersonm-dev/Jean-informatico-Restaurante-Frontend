<!-- src/views/restaurante/Configuracion/ConfiguracionWhatsappView.vue -->
<template>
      ...
      <div class="header-left">
        <div class="header-title">
          ...
          <div>
            ...
            <p class="text-muted">Gestiona las configuraciones de WhatsApp Business</p>
          </div>
        </div>
      </div>
        ...
        <table class="table" v-if="!loading">
          ...
          <tbody>
            <tr v-for="(item, index) in filteredItems" :key="item.id">
              ...
              <td>
                <span class="telefono-text">{{ item.telefono }}</span>
              </td>
              <td><code>{{ item.api_url }}</code></td>
              <td>
                <span class="api-key-text">••••{{ item.api_key?.slice(-4) || 'N/A' }}</span>
              </td>
              <td>
            ...
            <tr v-if="!filteredItems.length">
              <td colspan="8" class="text-center py-5">
                ...
                <p class="text-muted mt-2">No hay configuraciones de WhatsApp registradas</p>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal modal-lg">
        <div class="modal-header">
          <h3><i class="bi bi-whatsapp text-success"></i> {{ isEditing ? 'Editar Configuración' : 'Nueva Configuración' }}</h3>
              ...
              <div class="col-md-6">
                <div class="field-group">
                  ...
                  <input 
                    ...
                    required
                  >
                  <small class="help-text">Formato internacional con código de país</small>
                </div>
              </div>
            </div>
                ...
                <div class="field-group checkbox-group">
                  <label class="checkbox-label">
                    ...
                    <span class="toggle-text">
                      ...
                      {{ form.activo ? 'Configuración Activa' : 'Configuración Inactiva' }}
                    </span>
                  </label>
                </div>
              </div>
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-success" @click="guardarConfiguracion" :disabled="saving">
...
import { ConfiguracionWhatsappAPI, SedesAPI } from '@/services/api'
...
const filteredItems = computed(() => {
  ...
  if (filters.search) {
    ...
    result = result.filter(i =>
      ...
      i.telefono?.toLowerCase().includes(search) ||
      i.api_url?.toLowerCase().includes(search)
    )
  }
...
async function loadData() {
  loading.value = true
  try {
    const res = await ConfiguracionWhatsappAPI.list()
...
async function guardarConfiguracion() {
  ...
  if (!form.api_url?.trim()) {
    Swal.fire('Error', 'La API URL es obligatoria', 'error')
    return
  }
  if (!form.api_key?.trim()) {
    Swal.fire('Error', 'La API Key es obligatoria', 'error')
    return
  }
  ...
  try {
    ...
    if (isEditing.value) {
      await ConfiguracionWhatsappAPI.update(form.id, payload)
    } else {
      await ConfiguracionWhatsappAPI.create(payload)
    }
...
async function probarConfiguracion(item) {
  const result = await Swal.fire({
    ...
    text: `¿Deseas enviar un mensaje de prueba con la configuración "${item.nombre || 'WhatsApp'}"?`,
    icon: 'question',
  ...
  if (result.isConfirmed) {
    try {
      await ConfiguracionWhatsappAPI.test(item.id)
      Swal.fire('Éxito', 'Mensaje de prueba enviado correctamente', 'success')
      await loadData()
    } catch (error) {
      const msg = error.response?.data?.message || 'Error al enviar el mensaje de prueba'
      Swal.fire('Error', msg, 'error')
    }
  }
}
...
async function toggleEstado(item) {
  ...
  const result = await Swal.fire({
    ...
    text: `¿Estás seguro de ${accion} la configuración "${item.nombre || 'WhatsApp'}"?`,
    icon: 'warning',
    showCancelButton: true,
  ...
  if (result.isConfirmed) {
    try {
      await ConfiguracionWhatsappAPI.toggleStatus(item.id)
...
function verDetalle(item) {
  Swal.fire({
    title: `Configuración de WhatsApp - ${item.nombre || 'Sin nombre'}`,
    html: `
      <div style="text-align: left;">
        <p><strong>Sede:</strong> ${item.sede_nombre || item.sede_id}</p>
        <p><strong>Teléfono:</strong> ${item.telefono}</p>
        <p><strong>API URL:</strong> <code>${item.api_url}</code></p>
        <p><strong>API Key:</strong> ••••${item.api_key?.slice(-4) || 'N/A'}</p>
...
<style scoped>
/* Estilos iguales a ConfiguracionEmailView.vue */
...
.header-title i { font-size: 32px; color: #25D366; background: #e8f5e9; padding: 12px; border-radius: 12px; }
...
.telefono-text { font-weight: 600; color: #25D366; font-family: 'Courier New', monospace; }
.api-key-text { font-family: 'Courier New', monospace; color: #6b7280; }
...
.btn-success { background: #25D366; color: #ffffff; }
.btn-success:hover { background: #1da851; box-shadow: 0 4px 12px rgba(37,211,102,0.3); }
...
.btn-icon.btn-success { color: #25D366; border-color: #dcfce7; }
.btn-icon.btn-success:hover { background: #dcfce7; border-color: #25D366; }
...
.spinner { width: 40px; height: 40px; border: 4px solid #e5e7eb; border-top: 4px solid #25D366; border-radius: 50%; animation: spin 1s linear infinite; margin: 0 auto; }
...
.form-control:focus { outline: none; border-color: #25D366; box-shadow: 0 0 0 3px rgba(37,211,102,0.08); }
...
.checkbox-label input[type="checkbox"] { width: 18px; height: 18px; margin: 0; cursor: pointer; accent-color: #25D366; flex-shrink: 0; }
